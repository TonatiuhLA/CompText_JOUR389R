...
# Hard to decipher what the trend is. I'm not quite good with that anaylsis yet.
...
# Highest complaints in the morning when people are waking up. Makes sense but still interesting
...
No_Result <- SF |> 
  filter(disposition1 == "No_Dispo" | disposition1 == "Gone")
...
Serious_Result <- SF |> 
  filter(disposition1 == "Arrest" | disposition1 == "Cited" | disposition1 == "Criminal"
         | disposition1 == "SF Fire Dept Medical Staff engaged")
...
#Some minor variation through time but huge drop in May 2018. I wonder why? It slowly rises from there
...
SF |> 
  ...
  geom_bar(stat = "identity", show.legend = FALSE) +
  theme(axis.text.x = element_text(angle=90)) +
  labs(title = "Calls to SF PD About the Homeless by Year and Month", 
       subtitle = "Arrests Based on SF PD Service Call Data by Month 2017-2019",
       caption = "Graphic by Tonatiuh Leung-Arellano",
       y="Number of Calls",
       x="Year-Month")
...
# Benji answer - Complaints do vary by day. Weekdays share similar amounts but complaint numbers drop on weekends

SF <- SF |> 
  mutate(day_of_week = wday(call_date2, label = TRUE, abbr = FALSE, week_start = 1))

day_summary <- SF |> 
  count(day_of_week, name = "total_calls") |> 
  arrange(day_of_week)

```

# Graphic for total calls per day
```{r}

day_summary |> 
  ggplot(aes(x = day_of_week, y = total_calls, fill=total_calls)) +
  geom_bar(stat = "identity", show.legend = FALSE) +
  theme(axis.text.x = element_text(angle=90)) +
  labs(title = "Calls to SF PD About the Homeless by Day of Week", 
       subtitle = "Arrests Based on SF PD Service Call Data by Month 2017-2019",
       caption = "Graphic by Tonatiuh Leung-Arellano",
       y="Number of Calls",
       x="Weekday")
...
```{r}
SF <- SF |> 
  mutate(crime_all = str_c(crime_cleaned, crime3_cleaned, crime4_cleaned, sep = " "))
```
