...
```         
Benji Leung-Arellano
tleungar@terpmail.umd.edu
LinkedIn: https://www.linkedin.com/in/tonatiuhleungarellano/ 
ver. 9/2/2026
```
...
```{r}
#3: Identify title, gender and race of the highest paid position at NYT 
# max(NYT$Salary) -> reveals that max salary is: 350000
maxSalNYT <- NYT %>%
...
BusinessComp <- MediaBucks %>%
  filter(COMPANY == "Business")
BloombergComp <- MediaBucks %>%
  filter(COMPANY == "Bloomberg")

summary(BusinessComp$Salary)
summary(BloombergComp$Salary)


# Bloomberg is higher but that's because there is no company that fits
# the name "Business" in the company column, so it just returns N/A
```
